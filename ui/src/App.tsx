...
const dict: Record<Lang, Record<string, string>> = {
  tr: {
    ...
    compactHelp:
      "Modül sayısını otomatik azaltır: 0=Kapalı, 1=Az, 2=Orta (önerilen), 3=Agresif.",
    ...
    packSmallHelp:
      "Bu satır sayısından küçük modüller 'core' içinde birleştirilir. Örn: 120.",
    maxModules: "En fazla modül sayısı",
    maxModulesHelp:
      "Toplam modül sayısı bu sınırı aşarsa en küçük modüller birleştirilir (constants/core hariç).",
    minLines: "Modül başına minimum satır",
    minLinesHelp:
      "Bu eşiğin altındaki modüller başka modüllerle kaynaştırılır. Örn: 80.",
    targetMods: "Hedef modül sayısı",
    targetModsHelp:
      "Mümkünse modül sayısı buraya kadar düşürülür. Boşsa kapalıdır.",
    aiName: "Yapay zekâ ile paket adı öner",
    aiNameHelp:
      "Docstring ve üst düzey isimlere göre Ollama modelinden isim önerir.",
    ...
    instruction: "Talimat",
    instrPlaceholder:
      "Örn: Docstring ekle, tip ipuçlarını tamamla ve kullanılmayan importları kaldır.",
  ...
  en: {
    ...
    compactHelp:
      "Automatically reduce module count: 0=Off, 1=Low, 2=Medium (recommended), 3=Aggressive.",
    compactOffLabel: "Off",
    compactLowLabel: "Low",
    ...
    packSmallHelp:
      "Modules below this line count are merged into 'core'. e.g., 120.",
    maxModules: "Max modules",
    maxModulesHelp:
      "If exceeded, smallest modules get merged (except constants/core).",
    minLines: "Min lines per module",
    minLinesHelp:
      "Modules under this threshold are fused with others. e.g., 80.",
    targetMods: "Target module count",
    targetModsHelp:
      "Try to reduce to this count if possible. Empty = disabled.",
    ...
    instrPlaceholder:
      "e.g., Add docstrings, complete type hints, and remove unused imports.",
  ...
  de: {
    ...
    compactHelp:
      "Module automatisch reduzieren: 0=Aus, 1=Niedrig, 2=Mittel (empfohlen), 3=Aggressiv.",
    compactOffLabel: "Aus",
    ...
    maxModulesHelp:
      "Bei Überschreitung werden kleinste Module zusammengeführt.",
    ...
    aiNameHelp:
      "Vorschlag über Ollama basierend auf Docstrings & Top-Level-Namen.",
    ollamaUrl: "Ollama URL",
    ...
    instrPlaceholder:
      "z. B.: Docstrings hinzufügen, Typisierungen vervollständigen und unbenutzte Importe entfernen.",
  ...
  fr: {
    ...
    compactHelp:
      "Réduction auto des modules : 0=Off, 1=Faible, 2=Moyen (recommandé), 3=Agressif.",
    ...
    instruction: "Instruction",
    instrPlaceholder:
      "p. ex. : Ajouter des docstrings, compléter les annotations de type et supprimer les imports inutilisés.",
  ...
  es: {
    ...
    compactHelp:
      "Reduce automáticamente módulos: 0=Desactivado, 1=Bajo, 2=Medio (recomendado), 3=Alto.",
    ...
    instrPlaceholder:
      "p. ej.: Añade docstrings, completa las anotaciones de tipo y elimina imports no usados.",
  ...
  ru: {
    ...
    compactHelp:
      "Авто-сокращение модулей: 0=Выкл, 1=Низк, 2=Средний (рекомендуется), 3=Агрессивный.",
    ...
    maxModulesHelp:
      "Если превышено — объединять наименьшие (кроме constants/core).",
    ...
    instrPlaceholder:
      "напр.: Добавьте docstring-и, дополните подсказки типов и удалите неиспользуемые импорты.",
  ...
  ar: {
    ...
    compactHelp:
      "تقليل عدد الوحدات تلقائياً: 0=إيقاف، 1=منخفض، 2=متوسط (موصى به)، 3=عالٍ.",
    ...
    instrPlaceholder:
      "مثال: أضف توثيقات (docstrings)، أكمل تلميحات الأنواع، وأزل الواردات غير المستخدمة.",
  ...
  ja: {
    ...
    instruction: "指示",
    instrPlaceholder:
      "例：Docstring を追加、型ヒントを補完、未使用の import を削除。",
    langForAI: "説明言語（AI）",
...
function useUITranslations() {
  const [uiLang, setUiLang] = useState<Lang>(
    () => (localStorage.getItem("uiLang") as Lang) || "en",
  );
  useEffect(() => {
...
function Chip({ children }: { children: React.ReactNode }) {
  return (
    <span className="px-2 py-0.5 rounded-full text-xs border">{children}</span>
  );
}
function TooltipIcon({ text }: { text: string }) {
  return (
    ...
    >
      ...
      <span
        role="tooltip"
        className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-[135%] z-20 whitespace-nowrap rounded-md bg-black/90 text-white text-[11px] px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
      >
        {text}
      </span>
    </span>
...
export default function App() {
  ...
  async function doPlan() {
    ...
    if (files.length === 1) {
      ...
      setSinglePlan(data);
      setAiSuggested(data.ai_name || null);
      setMultiPlans(null);
    } else {
      ...
      const res = await fetch(`${API_BASE}/plan_multi`, {
        method: "POST",
        body: mfd,
      });
      const data = await res.json();
  ...
  async function doBuild() {
    ...
    if (files.length === 1) {
      ...
      fd.append("ollama_base_url", ollamaURL);
      fd.append("ollama_model", String(ollamaModel));
      const res = await fetch(`${API_BASE}/build_multi`, {
        method: "POST",
        body: fd,
      });
      const blob = await res.blob();
  ...
  async function doAiEdit() {
    ...
    if (files.length === 1) {
      ...
      fd.append("ollama_base_url", ollamaURL);
      fd.append("ollama_model", String(ollamaModel));
      const res = await fetch(`${API_BASE}/ai_edit`, {
        method: "POST",
        body: fd,
      });
      const data = await res.json();
      ...
      fd.append("ollama_base_url", ollamaURL);
      fd.append("ollama_model", String(ollamaModel));
      const res = await fetch(`${API_BASE}/ai_edit_multi`, {
        method: "POST",
        body: fd,
      });
      const blob = await res.blob();
      setEditedZipURL(URL.createObjectURL(blob));
      setEditProgress(100);
      setEditStatus("Completed");
    }
    _alive = false;
    setTimeout(() => setEditing(false), 350);
  }
  ...
  function downloadEditedSingle() {
    ...
    a.download = files[0].name.replace(/\.py$/i, "") + ".edited.py";
    a.click();
  ...
  const nodes: Node[] = useMemo(() => {
    ...
    const comps = singlePlan.plan.components as {
      module_name: string;
      lines: number;
    }[];
...
~${c.lines}`,
  ...
  const edges: Edge[] = useMemo(() => {
    if (!singlePlan) return [];
    return (singlePlan.plan.edges as { from: string; to: string }[]).map(
      (e, i) => ({
        id: String(i),
        source: e.from,
        target: e.to,
      ...
      <main className="max-w-5xl mx-auto p-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <section className="md:col-span-2 card-surface p-4">
            ...
            <label
              htmlFor="filePick"
              className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 cursor-pointer hover:bg-gray-50"
            >
              <Upload className="w-4 h-4" />
              {t("chooseFile")}
            </label>
            ...
            <div className="flex gap-2 mt-4 flex-wrap">
              <button
                onClick={doPlan}
                disabled={!files.length || planning}
                className="btn"
              >
              ...
              <button
                onClick={doBuild}
                disabled={!files.length || building}
                className="btn"
              >
              ...
              {downloadURL && (
                <a href={downloadURL} download className="btn">
                  ...
                  {files.length > 1 ? t("downloadPkgs") : t("downloadZip")}
                </a>
              )}
            </div>
                  ...
                  {multiPlans.map((pf) => (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-1">
                        {pf.plan.components.slice(0, 8).map((c: any) => (
                          <Chip key={c.module_name}>{c.module_name}</Chip>
                        ))}
                      </div>
                    </motion.div>
                  ))}
          ...
          <section className="card-surface p-4">
            ...
            <SettingRow label={t("compact")} help={t("compactHelp")}>
              ...
              >
                ...
                <option value={2}>
                  {t("compactMedLabel")} {t("recommended")}
                </option>
            ...
            <SettingRow label={t("packSmall")} help={t("packSmallHelp")}>
              <input
                type="number"
                value={packSmall}
                onChange={(e) => setPackSmall(Number(e.target.value))}
            ...
            <SettingRow label={t("aiName")} help={t("aiNameHelp")}>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={aiName}
                  onChange={(e) => setAiName(e.target.checked)}
                />
                <span>ON</span>
              </label>
            </SettingRow>
        ...
        <section className="card-surface p-4 mt-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2 flex-wrap mt-2">
              ...
              {files.length === 1
                ...
                : !!editedZipURL &&
                  !editing && (
                    <a href={editedZipURL} download className="btn">
                      <Download className="w-4 h-4" /> edited_files.zip
                    </a>
                  )}
            </div>
          </div>
          ...
          {files.length === 1 && (
            <div className="mt-4">
              ...
              <textarea
                className="w-full h-64 border rounded-lg p-2 font-mono text-xs"
                value={editedText}
                readOnly
              />
            </div>
          ...
          <section className="mt-6 grid md:grid-cols-3 gap-4">
            ...
            <div className="card-surface p-4">
              ...
              <div className="text-sm">
                {aiSuggested ? (
                  <Chip>{aiSuggested}</Chip>
                ) : (
                  <span className="opacity-60">—</span>
                )}
              </div>
              <div className="mt-3 text-xs opacity-60">{t("pkgNameGen")}</div>
            </div>
          </section>
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-2xl border border-white/20 bg-white/90 dark:bg-neutral-900/90 shadow-2xl p-6">
              ...
              <div className="mt-3 text-xs text-gray-500">
                This process may vary depending on your system speed and the
                model's response.
              </div>
            </div>
          </div>
